"""dd_notify platform for notify component."""
import asyncio
import logging
import os
...
async def async_get_service(hass, config, discovery_info=None):
    ...
    #     return
    #
    return DingDingNotificationService(
        hass,
        session,
...
token = 'd0176a199d2e0a162798d0e484da6570d1acce95cd0c2d65bd49aa5893de1526'
class DingDingNotificationService(BaseNotificationService):
    ...
    async def async_send_message(self, message, **kwargs):
        ...
        _LOGGER.info("async_send_message title: %s,message: %s ,data: %s", title, message, data)
        ...
        payload = {"msgtype": "text", "text": {"content": "空气污染, 是不一样的烟火"}}
        ...
        try:
            async with session.post(url, data=json.dumps(message), headers=headers) as res:
                _LOGGER.debug(
                    "Response on %s with data %s was %s", url, payload, await res.text()
                )
                _LOGGER.error("res.status: %s", res.status)
        except ClientError as err:
            _LOGGER.error("Error while uploading file message: %s", err)
